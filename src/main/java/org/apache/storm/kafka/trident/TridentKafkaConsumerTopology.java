...
public class TridentKafkaConsumerTopology {
        ...
        return tridentTopology.newDRPCStream("words", drpc)
                ...
                .filter(new BaseFilter() {
                    ...
                    public boolean isKeep(TridentTuple tuple) {
                        LOG.info("DRPC RESULT: {}:{}" + tuple.getString(0), tuple.getInteger(1));  // Used to show the DRPC results in the worker log. Useful for debugging
                        return true;
                    }
                });
        /**
    ...
    private static TridentState addTridentState(TridentTopology tridentTopology, ITridentDataSource tridentSpout) {
        ...
        /*
        spoutStream.filter(new BaseFilter() {
            @Override
            public boolean isKeep(TridentTuple tuple) {
                LOG.info("SPOUTSTREAM ISKEEP TUPLE: {}:{}" + tuple.getString(0), tuple.getInteger(1));
                return true;
            }
        });
         */
